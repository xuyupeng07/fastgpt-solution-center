...
> 区分：**公开的解决方案中心文章（报销助手、智能客服等，面向大众吸引客户）走另一条线**，见 `public-article.md`——那条线要"换名仍成立"，与本文件"换名失效"相反。
...
>
> **配图纪律**：回本周期用文字 + 表格表达（投入构成、月净收益、回本区间），**不要画伪造的"累计净现金流"曲线图**——那条曲线是编出来的，既难估量又不诚实。要配图，改用瀑布图讲"从投入如何一步步累积到净收益的构成"，或环形图讲"一次性投入 / 持续投入 / 收益"的结构。
...
> **三级编号**：一级「一、」、二级「1.1」、三级「1.1.1」。编号只锁层级与缩进，标题文字按内容定制。子章节用 `1.1`（如「1.1 现状业务梳理」「1.2 痛点因链」），再下一级用 `1.1.1`。
...
**图表选型（图型跟着数据的故事走，别用固定图型）**：私人定制有客户真实数据，图型更应丰富、更有冲击力，而不是每份方案都一张"前后对比柱状图"。先写"这张图的一句话结论"，再看结论落在哪种故事、选对应图型：

| 数据的故事 | 首选图型 | 售前方案示例 |
| --- | --- | --- |
| 分类对比 | 柱状图 / 横向条形图 | 各渠道咨询量、各环节耗时对比 |
| 随时间变化 | 折线图 / 面积图 | 上线后 N 个月采纳率爬坡 |
| 构成 / 占比 | 饼图 / 环形图 | 坐席时间分配、成本结构 |
| 多维打分 | 雷达图 | 方案在「效率/成本/质量/覆盖」四维评估 |
| 转化 / 流失 | 漏斗图 | 咨询→分诊→挂号→就诊 转化 |
| 来源去向 / 流转 | 桑基图 | 多渠道 → 能力 → 系统流转 |
| 相关 / 分布 | 散点图 | 处理时长 vs 满意度 |
| 累积桥接 | 瀑布图 | 从现状耗时一步步减掉节省，落到目标耗时 |
| 强度 / 矩阵 | 热力图 | 各科室 × 各时段咨询强度 |
| 排名 | 横向条形图 | 高频问题 Top N |

**三条纪律**：① 图型跟着故事走、同一份方案里图型不重复；② 数据真实可信、禁止"落地前 = 0"这类伪基线（没有真实"前"值就换占比/构成/漏斗/桑基/热力）；③ 禁止伪造"回本周期"曲线（见 4.12）。
